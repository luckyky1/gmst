...
% 两个月滑动平均数据
y = NaN(length(mean_temp_time_series) / 2, 1); % 初始化
time_dates_bimonthly = NaN(length(y), 1); % 两个月时间点
for i = 1:2:length(mean_temp_time_series) - 1
    y((i + 1) / 2) = mean(mean_temp_time_series(i:i+1), 'omitnan'); % 每两个月平均
    time_dates_bimonthly((i + 1) / 2) = mean(time_dates(i:i+1)); % 两个月的中间时间
end

% 消去季节影响
mo = NaN(6, 1); % 两个月为一组，总共6组
for i = 1:6
    stid = 660 + i; % 对应两个月平均后的时间点起点
    sgt = reshape(y(stid:6:stid + 6 * 29), [30, 1]); % 到2020年
    mo(i) = mean(sgt);
end

y_deseasoned = NaN(size(y));
for i = 1:length(y)
    month_idx = 1 + mod(i - 1, 6); % 两个月一组
    y_deseasoned(i) = y(i) - mo(month_idx);
end
...
figure3 = figure;
hold on;
plot(time_dates_bimonthly, y, 'LineWidth', 1.5); % 两个月平均时间序列
plot(time_dates_bimonthly,ys, 'Linewidth', 1.5);
scatter(time_dates_bimonthly(indices_above_p99_validw + 1), y(indices_above_p99_validw + 1), 'ro', 'filled');
datetick('x', 'mmm yyyy');
xlabel('时间', 'fontsize', 12);
ylabel('平均GMST (°C)', 'fontsize', 12);
title('全球温度异常时间序列', 'fontsize', 15);
grid on;
...
figure4 = figure;
hold on;
plot(time_dates_bimonthly, dydt, 'b', 'LineWidth', 1.5); % 两个月时间点
plot(time_dates_bimonthly, percentile_90w * ones(size(dydt)), '--k');
plot(time_dates_bimonthly, percentile_95w * ones(size(dydt)), '--m');
plot(time_dates_bimonthly, percentile_99w * ones(size(dydt)), '--r');
scatter(time_dates_bimonthly(indices_above_p99_validw), dydt(indices_above_p99_validw), 'ro', 'filled');
datetick('x', 'mmm yyyy');
xlabel('时间', 'fontsize', 12);
ylabel('d(GMST)/dt (°C/两月)', 'fontsize', 12);
title('全球温度变化率 (d(GMST)/dt) 时间序列', 'fontsize', 15);
grid on;
...
function txt = myupdatefcn(~, event_obj)  
    ...
    x_time = pos(1); % 提取x轴时间
    y_value = pos(2); % 提取y轴值
    start_month = datestr(x_time - 15, 'mmm yyyy'); % 前一个月
    end_month = datestr(x_time + 15, 'mmm yyyy'); % 后一个月
    txt = {['时间: ', start_month, ' 和 ', end_month], ['值: ', num2str(y_value)]};  
end
