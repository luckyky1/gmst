...
k= 1.36 #标准差倍数
#%% 数据读取
GMST_EEMD = pd.read_csv("../data/GMSL_byEEMD.csv",header=0,index_col=0)
GMST_EEMD.index = pd.to_datetime(GMST_EEMD.index)
GMST_ONI = pd.read_csv("../data/GMSL_bynoi.csv",header=0,index_col=0)
GMST_ONI.index = pd.to_datetime(GMST_ONI.index)
GMST = pd.read_excel("../data/GMSL.xlsx",header=None,index_col=0,names=['GMSL','loose_GMSL'])
...
# 设置子图标题和Y轴标签
axes[0].set_title("Analysis by EEMD Level Anomaly", fontsize=14)
axes[0].set_ylabel("Level Anomaly (mm)", fontsize=12)
...
# 设置子图标题和Y轴标签
axes[1].set_title("Analysis by ONI Level Anomaly", fontsize=14)
axes[1].set_ylabel("Level Anomaly (mm)", fontsize=12)
...
axes[2].set_title("Analysis by loose_GMSL Level Anomaly", fontsize=14)
axes[2].set_ylabel("Level Anomaly (mm)", fontsize=12)
...
axes[3].set_title("Analysis by GMSL Level Anomaly", fontsize=14)
axes[3].set_ylabel("Level Anomaly (mm)", fontsize=12)
axes[3].legend(loc='upper left')
# --- 6. 美化整个图表 ---
# 为整个图表添加一个主标题
fig.suptitle('Time Series Analysis of Global Mean Sea Level (GMSL)', fontsize=18, fontweight='bold')
